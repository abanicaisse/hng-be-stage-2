...
export class ImageGeneratorService {
  private static readonly LOCAL_IMAGE_PATH = path.join(
    process.cwd(),
    "cache",
    "summary.png"
  );
    ...
    try {
      ...
      topCountries.forEach((country, index) => {
        const gdp = country.estimatedGdp
          ? `$${(country.estimatedGdp / 1e9).toFixed(2)}B`
          : "N/A";
      ...
      if (this.USE_S3) {
        ...
        console.log(
          `[${new Date().toISOString()}] Summary image uploaded to S3`
        );
        return imageUrl;
      } else {
        // Save locally
        ...
        console.log(
          `[${new Date().toISOString()}] Summary image saved locally`
        );
